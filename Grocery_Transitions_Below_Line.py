...
import numpy as np
from matplotlib.ticker import FuncFormatter  # <-- needed

# --- Config ---
CSV_PATH = r"C:\Users\60848\OneDrive - Bain\Desktop\Project_Genome\casework\Woolworths\Grocery_Journeys_summary_Global_FE_Update.csv"
start_year = 2014
end_year = 2025
plot_label = "Grocery_transitions"
...
def plot_distribution(data1, data2, label1, label2, column, title, xlabel, outfile):
    ...
    plt.hist(vals1, bins=15, alpha=0.6, label=label1, density=True, color=COLOR_STAY)
    if not vals1.empty:
        m1 = vals1.mean()
    ...
    plt.hist(vals2, bins=12, alpha=0.6, label=label2, density=True, color=COLOR_FALL)
    if not vals2.empty:
        m2 = vals2.mean()
