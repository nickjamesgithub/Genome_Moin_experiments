...
missing = [c for c in need if c not in df_hc.columns]
if missing:
    raise KeyError(f"Missing columns in healthcare dataframe: {missing}")

# --- Build X, y with numeric coercion & strict finite filtering ---
y = pd.to_numeric(df_hc[RESPONSE], errors="coerce").replace([np.inf, -np.inf], np.nan)
X = df_hc[FEATURE_COLS].apply(pd.to_numeric, errors="coerce").replace([np.inf, -np.inf], np.nan)
...
plt.savefig("Healthcare_TSR_Drivers_RF.png", dpi=150);
plt.show()
...
try:
    ...
    plt.title("TSR Drivers — Healthcare (SHAP Beeswarm)")
    plt.tight_layout();
    plt.savefig("Healthcare_TSR_SHAP_Beeswarm.png", dpi=150, bbox_inches="tight"); plt.show()
    plt.show()

    # Violin-style
    plt.figure(figsize=(10, 7), dpi=150)
    shap.summary_plot(shap_values, X_pretty, plot_type="violin", max_display=25, show=False)
    plt.title("TSR Drivers — Healthcare (SHAP Violin)")
    plt.tight_layout();
    plt.savefig("Healthcare_TSR_SHAP_Violin.png", dpi=150, bbox_inches="tight");
    plt.show()
except ImportError:
